...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && safe_post('_action') === 'save_step1') {
    ...
    if (!in_array('agent', $allowed, true)) {
        $errors[] = '현재 권한으로는 에이전트를 생성할 수 없습니다.';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && safe_post('_action') === 'create_user') {
    if (!isset($_SESSION['create_agent_step1'])) {
        echo "<script>alert('❌ 1단계 정보가 없습니다. 다시 진행해주세요.'); window.location.href='create_agent.php';</script>";
        exit;
    }
    ...
    $allowed = allowed_child_roles($current_role);
    if (!in_array('agent', $allowed, true)) {
        echo "<script>alert('❌ 현재 권한으로는 에이전트를 생성할 수 없습니다.'); window.history.back();</script>";
        exit;
    }
    ...
    try {
        ...
        echo "<script>alert('✅ 에이전트 계정 생성 완료!'); window.location.href='a_agent_list.php';</script>";
        exit;
    } catch (Throwable $e) {
        $conn->rollback();
