...
if ($login_id <= 0) {
    echo "<p><?= t('error.login_required','Login is required.') ?></p>";
    return;
}
...
<div class="card" style="padding:16px;">
    <div style="display:flex; gap:12px; align-items:flex-end; flex-wrap:wrap;">
        <div>
            <div style="font-weight:700; font-size:18px;"><?= t('referral_settlement.title') ?></div>
<div style="color:#666; font-size:12px; margin-top:4px;"><?= t('referral_settlement.base_date') ?>: <strong><?= htmlspecialchars($selected_date) ?></strong> <?= t('referral_settlement.default_yesterday') ?></div>
        </div>

        <form method="get" style="margin-left:auto; display:flex; gap:8px; align-items:flex-end;">
            <div>
                <label style="display:block; font-size:12px; color:#666; margin-bottom:4px;"><?= t('referral_settlement.date_select') ?></label>
                <input type="text" class="date-picker" name="date" value="<?= htmlspecialchars($selected_date) ?>" autocomplete="off">
            </div>
            <button type="submit" class="btn" style="padding:9px 12px;"><?= t('referral_settlement.btn_search') ?></button>
        </form>
    </div>
    ...
    <div style="margin-top:14px; overflow:auto;">
        <table class="table" style="width:100%; min-width:680px;">
            <thead>
                <tr>
                    <th style="white-space:nowrap;"><?= t('referral_settlement.table.date') ?></th>
                    <th style="white-space:nowrap;"><?= t('referral_settlement.table.referral_id') ?></th>
                    <th style="white-space:nowrap;"><?= t('referral_settlement.table.total_income') ?></th>
                    <th style="white-space:nowrap;"><?= t('referral_settlement.table.settlement_amount') ?></th>
                </tr>
            </thead>
            <tbody>
            <?php if (empty($rows)): ?>
...
<!--     <div style="margin-top:10px; font-size:12px; color:#777; line-height:1.5;">
        <?= t('referral_settlement.note1') ?><br>
        <?= t('referral_settlement.note2') ?><br>
