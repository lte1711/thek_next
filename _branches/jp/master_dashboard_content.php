...
$no_data_label = function_exists('t') ? t('common.no_data') : 'No data';
// 로그인 여부 확인
...
// 디버그 메시지
$debug_msg = t('master_dashboard.base_date') . ': ' . $target_date
           . ' / ' . t('master_dashboard.total_agents') . ': ' . count($agent_sales)
           . ' / ' . t('master_dashboard.detail_agents') . ': ' . count($detail_data);

// 안전 기본값 처리
if (empty($agent_sales)) {
    $agent_sales = [$no_data_label => 0];
}
if (empty($detail_data)) {
    $detail_data = [$no_data_label => [['amount' => 0, 'date' => $target_date]]];
}
?>
...
<div class="gm-wrap">
  <div class="gm-grid">
    <div class="gm-card">
      <div class="gm-card-title"><?= t('master_dashboard.sum_title') ?></div>
      <div class="gm-card-subtitle"><?= t('master_dashboard.base_date') ?>: <?= htmlspecialchars($target_date) ?></div>
      <div class="gm-chart-box"><canvas id="agentDividendChart"></canvas></div>
    </div>
    <div class="gm-card">
      <div class="gm-card-title"><?= t('master_dashboard.detail_title') ?></div>
      <div class="gm-card-subtitle"><?= t('master_dashboard.base_date') ?>: <?= htmlspecialchars($target_date) ?></div>
      <div class="gm-chart-box"><canvas id="agentDetailChart"></canvas></div>
    </div>
  </div>
...
<script>
  ...
  new Chart(document.getElementById('agentDetailChart'), {
    ...
    data: {
      labels: barLabels,
      datasets: [{ label: <?= json_encode(t('master_dashboard.chart.dividend_amount')) ?>, data: barData, backgroundColor: barColors }]
    },
    options: {
      plugins: { legend: { display:false } },
      scales: {
        y: { beginAtZero:true, title: { display:true, text: <?= json_encode(t('master_dashboard.chart.dividend_amount_won')) ?> } },
        x: { title: { display:true, text: <?= json_encode(t('master_dashboard.chart.agent_date')) ?> } }
      }
    }
  });
</script>