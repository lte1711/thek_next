...
<div class="v2-wrap">
  <div class="v2-head">
    ...
    <div class="card mini-cal">
      ...
      <table class="cal">
        <thead>
          <tr><th><?= t('calendar.sun_short','S') ?></th><th><?= t('calendar.mon_short','M') ?></th><th><?= t('calendar.tue_short','T') ?></th><th><?= t('calendar.wed_short','W') ?></th><th><?= t('calendar.thu_short','T') ?></th><th><?= t('calendar.fri_short','F') ?></th><th><?= t('calendar.sat_short','S') ?></th></tr>
        </thead>
        <tbody>
          <tr>
          <?php
      ...
      <div style="margin-top:10px; font-size:12px; color:#666;">
        ...
        <?php else: ?>
          <span style="margin-left:6px; color:#2b6cb0;">(<?= t('status.unsettled','Unsettled') ?>)</span>
        <?php endif; ?>
      </div>
    </div>
    ...
    <div class="card main-area">
      <p class="v2-title" style="margin:0;"><?= t('title.group_settlement_v2','Group Settlement v2') ?></p>
      ...
      <div style="margin-top:14px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
        <div style="flex:1; min-width:240px;">
          ...
          <select onchange="if(this.value) location.href=this.value"
            ...
            <!-- ✅ 기본은 항상 '전체'가 선택되도록 ("<?= t('common.select', '선택하세요') ?>" 제거) -->
            <option value="<?= $all_url ?>" <?= ($view !== 'list' ? 'selected' : '') ?>><?= t('common.all','All') ?></option>
        ...
        <div style="display:flex; gap:8px; align-items:center;">
          ...
          <form method="post" action="settle_confirm_v2.php" style="margin:0;" onsubmit="return confirm('<?= t('confirm.sent_finalize', '정말로 SENT 확정하시겠습니까?\n(현재 화면 범위의 pending 항목이 sent로 변경됩니다)') ?>');">
      ...
      <?php if ($view === 'all'): ?>
        ...
        <div style="margin-top:14px;">
          <div style="font-weight:800; margin-bottom:10px;">
            <?= q($date) ?> <?= q($level_kor) ?> <?= t('label.settlement_table','정산 테이블') ?>
          ...
          <?php if (empty($table_rows)): ?>
            <div class="placeholder">
              <?= t('msg.no_settlement_table_data','정산 테이블 데이터가 없습니다.') ?>
              <?php if ($level !== 'admin' && $target === ''): ?>
                <br><?= t('msg.parent_selection_required','(상위 선택이 필요합니다)') ?>
              <?php endif; ?>
            </div>
          <?php else: ?>
            <table class="tbl">
              <thead>
                <tr>
                  <th style="width:220px;"><?= t('table.username','아이디') ?></th>
                  <th><?= t('table.codepay','코드페이') ?></th>
                  <th style="width:190px; text-align:center;"><?= t('table.progress','진행상태') ?></th>
                  <th style="width:180px; text-align:right;"><?= t('table.settlement_amount','정산 금액') ?></th>
                </tr>
              </thead>
              <tbody>
                <?php foreach ($table_rows as $r): ?>
                  ...
                  <tr>
                    ...
                    <td style="text-align:center; font-size:12px; color:#555;">
                      <?php if ($done): ?>
                        <span style="color:#666;"><?= t('status.completed','완료') ?></span>
                      <?php else: ?>
                        <span style="color:#2b6cb0;"><?= t('status.incomplete','미완료') ?></span><br>
                        <span style="color:#666;"><?= t('status.pending','pending') ?> <?= number_format($rp_pending) ?> (<?= number_format($rp_pa, 2) ?>)</span>
                      <?php endif; ?>
                    </td>
              ...
              <tfoot>
                <tr>
                  <td colspan="3" style="text-align:right; font-weight:800;"><?= t('common.total','합계') ?></td>
      ...
      <?php else: ?>
        ...
        <?php if ($nl === null): ?>
          <div class="placeholder">
            <?= t('msg.lowest_level','✅ 최하위 단계입니다. (더 이상 하위 리스트가 없습니다)') ?><br>
            <?= t('msg.click_all_to_view_settlement_table','“전체”를 누르면 정산 테이블이 표시됩니다.') ?>
          </div>
        <?php else: ?>

          <div style="margin-top:14px;">
            <div style="font-weight:800; margin-bottom:10px;">
              <?= q($date) ?> <?= q($nl_kor) ?> <?= t('common.downline_list','하위 리스트') ?>
            </div>

            <?php if (!empty($need_select_parent)): ?>
              <div class="placeholder"><?= t('msg.select_admin_first','어드민을 먼저 선택해주세요.') ?></div>

            <?php elseif (empty($child_rows)): ?>
              <div class="placeholder"><?= t('msg.no_child_data_for_date','해당 날짜에 하위 데이터가 없습니다.') ?></div>
            ...
            <?php else: ?>
              <table class="tbl">
                <thead>
                  <tr>
                    <th style="width:220px;"><?= t('table.username','아이디') ?></th>
                    <th style="width:160px; text-align:right;"><?= t('table.settlement_amount','정산 금액') ?></th>
                    <th style="width:140px; text-align:center;"><?= t('common.move','이동') ?></th>
                  </tr>
                </thead>
                <tbody>
                  <?php foreach ($child_rows as $r): ?>
                    ...
                    <tr>
                      ...
                      <td style="text-align:center;">
                        <a href="<?= $url ?>" style="text-decoration:none; font-weight:700;"><?= t('common.view_downline','하위 보기') ?></a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
                <tfoot>
                  <tr>
                    <td style="text-align:right; font-weight:800;"><?= t('common.total','합계') ?></td>
