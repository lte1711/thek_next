...
// === language helper (no t() here; layout handles i18n) ===
$__cm_lang = $_GET['lang'] ?? ($_SESSION['lang'] ?? 'ko');
$__cm_lang = strtolower(trim((string)$__cm_lang));
if (!in_array($__cm_lang, ['ko','ja','en'], true)) $__cm_lang = 'ko';
function __cm_tr(string $ko, string $ja, string $en): string {
    global $__cm_lang;
    if ($__cm_lang === 'ja') return $ja;
    if ($__cm_lang === 'en') return $en;
    return $ko;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && safe_post('_action') === 'save_step1') {
    ...
    if (!in_array('master', $allowed, true)) {
        $msg = __cm_tr('❌ 현재 권한으로는 마스터를 생성할 수 없습니다.','❌ 現在の権限ではマスターを作成できません。','❌ You do not have permission to create a master.');
    ...
    $dup_stmt->close();
    if ((int)($dup['cnt'] ?? 0) > 0) {
        $msg = __cm_tr('❌ 이미 존재하는 ID 또는 Email입니다.','❌ すでに存在するIDまたはEmailです。','❌ ID or Email already exists.');
        echo "<script>alert(" . json_encode($msg) . "); window.history.back();</script>";
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && safe_post('_action') === 'create_user') {
    if (!isset($_SESSION['create_master_step1'])) {
        $msg = __cm_tr('❌ 1단계 정보가 없습니다. 다시 진행해주세요.','❌ 1段階の情報がありません。もう一度やり直してください。','❌ Step 1 information is missing. Please try again.');
        echo "<script>alert(" . json_encode($msg) . "); window.location.href='create_master.php';</script>";
        exit;
    }
    ...
    try {
        ...
        $msg = __cm_tr('✅ 마스터 계정 생성 완료!','✅ マスターアカウント作成完了！','✅ Master account created successfully!');
        echo "<script>alert(" . json_encode($msg) . "); window.location.href='a_master_list.php';</script>";
        exit;
    } catch (Throwable $e) {
        $conn->rollback();
...
// ===== GET: render =====
$page_title = __cm_tr("마스터 생성", "マスター作成", "Create Master");
