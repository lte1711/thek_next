...
if (!isset($_SESSION['user_id'])) {
    ...
    echo t('common.login_required','Login is required.');
    exit;
}
...
if (!$is_gm) {
    ...
    echo t('err.gm_only_settle', 'GM only can settle.');
    exit;
}
...
if (!$dt || $dt->format('Y-m-d') !== $settle_date) {
    ...
    echo t('err.invalid_date_format', '잘못된 날짜 형식입니다. (YYYY-MM-DD)');
    exit;
}

if ($action !== 'download_only' && $action !== 'confirm_sent') {
    http_response_code(400);
    echo t('err.invalid_request', '잘못된 요청입니다.');
    exit;
}
...
if ($wanted_role === null) {
    http_response_code(400);
    echo t('err.invalid_level','Invalid level.');
    exit;
}
...
if ($parent_filter_col !== null && $target === '') {
    http_response_code(400);
    echo t('err.target_required', '상위 선택(target)이 필요합니다.');
    exit;
}
...
try {
    ...
    if (empty($raw_items)) {
        ...
        echo t('settlement.already_completed','Already settled.') . ': ' . $settle_date;
        exit;
    }
    ...
    echo t('settlement.sent_confirmed','SENT confirmed.');
    exit;

} catch (Throwable $e) {
    $conn->rollback();
    http_response_code(500);
    echo t('err.settlement_failed_prefix','Settlement failed: ') . $e->getMessage();
    exit;
}
