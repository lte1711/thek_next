...
// 권한: GM 또는 본인만
if (!$is_gm && !$is_self) {
    echo "<script>alert(" . json_encode(t('msg.no_permission', '❌ 권한이 없습니다.')) . "); window.history.back();</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && safe_post('_action') === 'save_step1') {
    ...
    $errors = [];
    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) $errors[] = t('err.check_email');
    if ($wallet_address === '') $errors[] = t('err.wallet_required');

    // ✅ 비밀번호는 본인일 때만 허용(입력 시에만 변경)
    if (!$is_self && $password !== '') {
        $errors[] = t('err.password_self_only');
    }

    if ($errors) {
        echo "<script>alert(" . json_encode(t('err.input_error_title') . "\n- " . implode("\n- ", $errors)) . "); window.history.back();</script>";
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && safe_post('_action') === 'update_user') {
    if (!isset($_SESSION['investor_edit_step1']) || !is_array($_SESSION['investor_edit_step1'])) {
        echo "<script>alert(" . json_encode(t('err.step1_missing')) . "); window.location.href=" . json_encode("investor_edit_broker.php?id=".$target_id) . ";</script>";
        exit;
    }
    ...
    try {
        ...
        $conn->rollback();
        echo "<script>alert(" . json_encode(t('err.processing_error_prefix') . $e->getMessage()) . "); window.history.back();</script>";
        exit;
    }
}
...
$page_title = 'page.investor_edit_broker';
