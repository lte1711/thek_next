...
if (!isset($_SESSION['user_id'], $_SESSION['role'])) {
    ...
    echo json_encode(['error' => t('error.unauthorized', 'Unauthorized')]);
    exit;
}
...
if ($session_role !== 'gm') {
    ...
    echo json_encode(['error' => t('error.forbidden', 'Forbidden')]);
    exit;
}
...
/**
 * 2) 직계 하위 생성 (new 버튼)
 * - parent_id를 기준으로 t('label.direct_child_role', '바로 아래 role')만 생성시키는 모드
...
if ($action === 'create_child') {
    ...
    if ($parent_id <= 0 || $username === '' || $password === '') {
        ...
        echo json_encode(['error' => t('err.required_missing_parent_user_pass', 'Required values missing (parent_id/username/password)')]);
        exit;
    }
    ...
    if (!$parent) {
        http_response_code(400);
        echo json_encode(['error' => t('err.parent_not_found', '상위 사용자를 찾을 수 없습니다.')]);
        exit;
    }
    ...
    if ($new_role === '') {
        http_response_code(400);
        echo json_encode(['error' => t('err.role_new_not_supported', '이 ROLE에서는 new(직계하위 생성)을 지원하지 않습니다.')]);
        exit;
    }
    ...
    if ($st->get_result()->fetch_assoc()) {
        ...
        echo json_encode(['error' => t('err.username_exists', '이미 존재하는 아이디입니다.')]);
        exit;
    }
    $st->close();
    ...
    try {
        ...
        echo json_encode(['error' => t('err.account_create_failed', '계정 생성 실패'), 'message' => $e->getMessage()]);
        exit;
    }
}
...
if ($action === 'create_referral') {
    ...
    if ($investor_id <= 0 || $username === '' || $password === '') {
        http_response_code(400);
        echo json_encode(['error' => t('err.required_missing_investor_user_pass', '필수 값 누락(investor_id/username/password)')]);
        exit;
    }
    ...
    if (!$inv || strtolower($inv['role']) !== 'investor') {
        http_response_code(400);
        echo json_encode(['error' => t('err.ref_create_investor_only', '추천인 생성은 investor에서만 가능합니다.')]);
        exit;
    }
    ...
    if ($agent_id <= 0) {
        http_response_code(400);
        echo json_encode(['error' => t('err.referrer_parent_agent_missing', '상위(agent) 연결이 없어 추천인 생성 불가')]);
        exit;
    }
    ...
    try {
        ...
        $conn->rollback();
        http_response_code(500);
        echo json_encode(['error' => t('err.ref_create_failed', '추천인 생성 실패'), 'message' => $e->getMessage()]);
        exit;
    }
}
