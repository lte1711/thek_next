...
if (!function_exists('t')) {
    ...
    if (file_exists($i18n1)) {
        require_once $i18n1;
    } elseif (file_exists($i18n2)) {
        require_once $i18n2;
    }
}
...
if ($conn->connect_error) {
    die(t('err.db_connect_failed_prefix') . $conn->connect_error);
}
...
$chk_sql = "SELECT 1 FROM user_transactions
            WHERE user_id=?
              ...
              AND (COALESCE(xm_total,0) + COALESCE(ultima_total,0)) > 0
            LIMIT 1";
...
if (!$has_withdrawal) {
    // ✅ 메시지 출력 후 대시보드로 이동
    echo "<script>alert(" . json_encode(t('profit_share.alert.withdraw_not_done')) . "); location.href='investor_dashboard.php';</script>";
    exit;
}
...
$referral_username = getReferrerUsername($user_id, $conn) ?? 'TheK_KO';
        ...
        if (!$insert_stmt->execute()) {
            echo "<script>alert(" . json_encode(t('profit_share.err.dividend_save_prefix')) . " + ' ' + " . json_encode($insert_stmt->error) . ");</script>";
        }
        $insert_stmt->close();
            ...
            try {
                ...
                if ($has_batches && $has_items) {
                    ...
                    if (!$batch_stmt->execute()) {
                        throw new Exception(t('error.codepay_batches_save_failed_prefix','codepay_payout_batches save failed') . ": " . $batch_stmt->error);
                    }
                    $batch_stmt->close();
                    ...
                    if (!$already_exists) {
                        ...
                        if (!$items_stmt->execute()) {
                            throw new Exception(t('error.codepay_items_save_failed_prefix','codepay_payout_items save failed') . ": " . $items_stmt->error);
                        }
                        $items_stmt->close();
                    }
                }
        ...
        if ($update_stmt->execute()) {
            echo "<script>
                    alert(" . json_encode(t('profit_share.alert.settled_ok')) . ");
                    window.location.href = 'investor_profit_share.php?user_id=" . (int)$user_id . "';
                  </script>";
            exit;
        } else {
            echo "<script>
                    alert(" . json_encode(t('profit_share.err.settle_prefix')) . " + ' ' + " . json_encode($update_stmt->error) . ");
                    window.location.href = 'investor_profit_share.php?user_id=" . (int)$user_id . "';
                  </script>";
            exit;
        }
...
$wallet_stmt->execute();
$wallet = $wallet_stmt->get_result()->fetch_assoc()['wallet_address'] ?? t('msg.no_wallet');
$wallet_stmt->close();
...
// === 레이아웃 적용 ===
$page_title   = t('page.investor_profit_share');
