...
if (!$is_gm) {
    ...
    echo t('error.gm_only_settle','Only GM can settle.');
    exit;
}
...
if (!$dt || $dt->format('Y-m-d') !== $settle_date) {
    ...
    echo t('error.invalid_date_format','Invalid date format (YYYY-MM-DD)');
    exit;
}
...
try {
    ...
    if (!empty($row) && (int)$row['cnt'] > 0) {
        ...
        echo t('msg.already_settled_date','Already settled date:') . ' ' . $settle_date;
        exit;
    }
...
echo t('msg.gm_settle_saved','GM settlement saved') . "\n- " . t('field.date','Date') . ": {$settle_date}\n- " . t('field.total','Total') . ": " . number_format($sales_amount, 2);

} catch (Throwable $e) {
    $conn->rollback();
    http_response_code(500);
    echo t('error.gm_settle_save_failed_prefix','GM settle save failed:') . ' ' . $e->getMessage();
}
