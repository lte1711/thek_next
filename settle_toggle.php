...
try {
    ...
    if (!$tx) {
        throw new Exception("거래를 찾을 수 없습니다.");
    }
    ...
    if (!$upd->execute()) {
        throw new Exception("정산 업데이트 실패: " . $upd->error);
    }
    $upd->close();
    ...
    if ($row) {
        ...
        if (!$insb->execute()) {
            throw new Exception("배치 생성 실패: " . $insb->error);
        }
    ...
    if ($batch_id && $amount > 0 && $codepay_address !== null && trim($codepay_address) !== '') {
        ...
        if (!$exists) {
            ...
            if (!$insi->execute()) {
                throw new Exception("지급 아이템 생성 실패: " . $insi->error);
            }
            $insi->close();
        }
    }
