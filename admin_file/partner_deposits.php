...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action'])) {
    try {
        if ($_POST['action'] === "create") {
            ...
            $stmt = $conn->prepare("INSERT INTO partner_deposits (region, deposit_amount, deposit_time) VALUES (?, ?, ?)");
            $stmt->bind_param("sds", $region, $deposit_amount, $deposit_time);
            $stmt->execute();
            $stmt->close();
            ...
            $stmt = $conn->prepare("UPDATE partner_deposits SET region=?, deposit_amount=?, deposit_time=? WHERE id=?");
            $stmt->bind_param("sdsi", $region, $deposit_amount, $deposit_time, $id);
            $stmt->execute();
            $stmt->close();
            ...
            $stmt = $conn->prepare("DELETE FROM partner_deposits WHERE id=?");
...
$result = $conn->query("SELECT * FROM partner_deposits ORDER BY id ASC");
?>

<?php admin_render_header('파트너 입금 관리 (partner_deposits)'); ?>
...
<div class="card">
  <h2 style="margin-top:0;">파트너 입금 등록</h2>
  <form method="POST">
    <?= csrf_input() ?>
    ...
    <div class="form-row">
      ...
      <div>
        <label>입금 시간</label>
        <input type="datetime-local" name="deposit_time" required>
      </div>
...
<div class="card">
  <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap;">
    <h2 style="margin:0;">파트너 입금 목록</h2>
      ...
      <table>
        ...
        <?php
          ...
          while ($row = $result->fetch_assoc()):
            ...
            $dt = (string)($row['deposit_time'] ?? '');
            $dt_value = htmlspecialchars(str_replace(' ', 'T', $dt));
        ?>
          <tr>
            ...
            <td><input form="<?= $f_upd ?>" type="datetime-local" name="deposit_time" value="<?= $dt_value ?>"></td>
