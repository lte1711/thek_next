...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    if ($_POST['action'] === "create") {
        $stmt = $conn->prepare("INSERT INTO vietnam_progressing (title, description, progress_date, status) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $_POST['title'], $_POST['description'], $_POST['progress_date'], $_POST['status']);
        $stmt->execute();
        $stmt->close();

    } elseif ($_POST['action'] === "update" && isset($_POST['id'])) {
        $stmt = $conn->prepare("UPDATE vietnam_progressing SET title=?, description=?, progress_date=?, status=? WHERE id=?");
        $stmt->bind_param("ssssi", $_POST['title'], $_POST['description'], $_POST['progress_date'], $_POST['status'], $_POST['id']);
        $stmt->execute();
        $stmt->close();

    } elseif ($_POST['action'] === "delete" && isset($_POST['id'])) {
        $stmt = $conn->prepare("DELETE FROM vietnam_progressing WHERE id=?");
        ...
        foreach ($_POST['ids'] as $id) {
            $id = intval($id);
            $conn->query("DELETE FROM vietnam_progressing WHERE id=$id");
        }
    }
}

// ✅ 목록 불러오기
$result = $conn->query("SELECT * FROM vietnam_progressing ORDER BY id ASC");
?>

?>
<?php admin_render_header('베트남 진행 상황 관리 (vietnam_progressing)'); ?>
<h2>베트남 진행 상황 등록</h2>
    <form method="POST">
    <?= csrf_input() ?>
