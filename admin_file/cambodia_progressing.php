...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    if ($_POST['action'] === "create") {
        ...
        $stmt = $conn->prepare("INSERT INTO cambodia_progressing (user_id, tx_date, pair, deposit_status, withdrawal_status, profit_loss, notes, settled_by, settled_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("UPDATE cambodia_progressing SET user_id=?, tx_date=?, pair=?, deposit_status=?, withdrawal_status=?, profit_loss=?, notes=?, settled_by=?, settled_date=? WHERE id=?");
        ...
        $id = intval($_POST['id']);
        $stmt = $conn->prepare("DELETE FROM cambodia_progressing WHERE id=?");
        ...
        foreach ($_POST['ids'] as $id) {
            $id = intval($id);
            $conn->query("DELETE FROM cambodia_progressing WHERE id=$id");
        }
    }
}

// ✅ 목록 불러오기
$result = $conn->query("SELECT * FROM cambodia_progressing ORDER BY id ASC");
?>

?>
<?php admin_render_header('캄보디아 진행 상황 관리 (cambodia_progressing)'); ?>
<h2>캄보디아 진행 상황 등록</h2>
    <form method="POST">
    <?= csrf_input() ?>
