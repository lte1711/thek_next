...
if ($user_id <= 0) { echo "<script>alert('❌ 잘못된 접근입니다.'); window.history.back();</script>"; exit; }
...
if ($member_id > 0) {
    ...
    if ($stmt_user->execute()) {
        echo "<script>
                alert('✅ 회원(ID: {$member_id}) 삭제 완료!');
                window.location.href = '{$return_url}';
              </script>";
    } else {
        echo "<script>
                alert('❌ 삭제 오류: " . $stmt_user->error . "');
                window.location.href = '{$return_url}';
              </script>";
    }
...
try {
    ...
    echo "<script>alert('❌ 삭제 실패: " . addslashes($e->getMessage()) . "'); window.history.back();</script>";
    exit;
}
// ===== 삭제 처리 끝 =====
