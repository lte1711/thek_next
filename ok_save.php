...
try {
  ...
  if ((int)($txrow['ext'] ?? 0) !== 1) throw new Exception('아직 외부 진행 확인 전입니다. (external_done_chk=0)');
  ...
  if (!$ready_has_tx_id) {
    throw new Exception("{$table_ready}.tx_id 컬럼이 없습니다. (DB 마이그레이션 먼저 적용 필요)");
  }
...
$conn->commit();
  ...
  echo json_encode(['success'=>true,'message'=>'OK 처리 완료 (봇 진행중)','ready_id'=>$ready_id]);
