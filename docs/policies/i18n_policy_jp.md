...
본 정책은 **정합성일관성재발 방지**를 최우선 목표로 한다.
...
- 기본 언어는 **일본어(`ja`)** 로 한다.
- `` 값이 다음 조건 중 하나라도 만족하지 못하면 기본값 `ja`를 사용한다.

### 기본값 적용 조건
- `` 파라미터가 존재하지 않을 경우
- `` 값이 `ko`, `ja`, `en` 중 하나가 아닐 경우
- 세션/쿠키 값이 비정상적인 경우

```php
// pseudo rule
if (!in_array(, ['ko', 'ja', 'en'])) {
     = 'ja';
}
```
...
- 특정 언어 파일에 키가 존재하지 않을 경우:
  1 동일 키의 **기본 언어(`ja`)** 값으로 fallback
  2 그래도 없을 경우, 키를 그대로 노출 (`[[KEY_NAME]]`)
...
```php
// allowed
require_once ['DOCUMENT_ROOT'] . '/_branches/jp/includes/i18n.php';
...
## 10. QA 체크리스트 (Mixed Language Regression)

아래 항목은 릴리즈 전/후 필수 확인 항목이다.

### 10.1 언어 강제 전환 테스트
- `?lang=ja` / `?lang=ko` / `?lang=en` 각각에서 다음 요소가 단일 언어로만 노출되는지 확인
  - 메뉴/헤더
  - 페이지 타이틀
  - 테이블 컬럼명/버튼 라벨
  - 빈 데이터 표시 문구(예: "-", "No data")

### 10.2 혼용 언어 검출 규칙
- 한 화면에서 서로 다른 언어(한글/일본어/영어)가 동시에 노출되면 실패로 판정한다.
- 특히 다음 영역은 혼용이 자주 발생하므로 우선 확인한다.
  - 글로벌 메뉴/사이드바
  - 공용 헤더 타이틀
  - 공용 버튼(저장/취소/검색)
  - 리스트 페이지의 상태 뱃지/라벨
...
### 10.3 누락 키 확인
- 번역 키가 누락된 경우 빈 문자열로 출력하지 않는다.
- `[[KEY_NAME]]` 형태로 노출되는지 확인한다. (누락을 즉시 발견하기 위함)

### 10.4 기본값/폴백 확인
- `lang` 파라미터가 없거나 잘못된 값일 때 기본값 `ja`가 적용되는지 확인한다.
- 특정 언어에서 누락 키 발생 시 기본 언어(`ja`)로 폴백되는지 확인한다.
