...
if ($chk) {
    ...
    $chk->close();
    if (!$exists) {
        die('잘못된 사용자입니다. (user_id=' . htmlspecialchars((string)$user_id) . ')');
    }
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['xm_value'])) {
    ...
    if ($tx_id <= 0) {
        die("tx_id 생성 실패 (user_transactions INSERT 이후 insert_id가 0)");
    }
...
do {
    ...
    // 미완료 사유는 모두 표시
    if ($w === 0) $blocked_reasons[] = "출금 미완료";
    if ($s === 0) $blocked_reasons[] = "정산 미완료";
    if ($d === 0) $blocked_reasons[] = "배당 미완료";
