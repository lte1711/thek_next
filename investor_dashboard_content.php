<?php
// 직접 URL 접근 차단 (반드시 investor_dashboard.php -> layout.php 통해서만 렌더)
...
<?php if ($rejecting_count > 0): ?>
  <div style="max-width:980px; margin:12px auto 18px; padding:12px 14px; border:1px solid #f1c40f; background:#fff9e6; border-radius:10px;">
    <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:12px;">
      <div>
        <div style="font-weight:800; margin-bottom:6px;">⚠️ Rejecting 처리 필요: <b><?= (int)$rejecting_count ?></b>건</div>
      ...
      <div style="white-space:nowrap;">
        <a href="profit_share.php?user_id=<?= (int)$current_user_id ?>&region=korea" class="toggle on" style="display:inline-block; padding:8px 12px; text-decoration:none;">거래내역에서 처리</a>
      </div>
    </div>
    <div style="margin-top:8px; color:#6b6b6b; font-size:12px;">
      ※ 거래내역에서 <b>재진행</b>으로 Rejecting을 해제한 뒤, 다시 <b>ON</b>을 눌러 진행완료를 전송하세요.
    </div>
  </div>
<?php endif; ?>

<section class="content-area">
    <h2 style="text-align:center;">라운드별 수익 (일주일간 날짜별)</h2>
    <div class="chart-box"><canvas id="profitChart"></canvas></div>

    <h2 style="text-align:center;">월별 매출 내역</h2>
