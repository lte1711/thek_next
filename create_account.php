...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (safe_post('_action') === 'save_step1')) {
    ...
    if (!$is_edit_post && $data['password'] === '') $errors[] = "비밀번호(password)는 필수입니다.";
    ...
    if (!$is_edit_post) {
        ...
        if ($edit_id <= 0) {
            echo "<script>alert('❌ 수정 대상 정보가 없습니다.'); window.history.back();</script>";
            exit;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (safe_post('_action') === 'create_user')) {
    if (!isset($_SESSION['create_account_step1'])) {
        echo "<script>alert('❌ 1단계 정보가 없습니다. 다시 시도해주세요.'); window.location.href=" . json_encode($redirect_url) . ";</script>";
        exit;
    }
    ...
    if ($new_role === 'gm' && $target_role !== 'gm') {
        echo "<script>alert('❌ GM 역할은 선택할 수 없습니다.'); window.history.back();</script>";
        exit;
    }
    ...
    if ($req_role !== null && $new_role !== 'admin') {
        ...
        if ($sponsor_id_raw === '' || !ctype_digit($sponsor_id_raw)) {
            ...
            if (!$sr) {
                echo "<script>alert('❌ 선택한 상위 사용자를 찾을 수 없습니다.'); window.history.back();</script>";
                exit;
            }
            if (strtolower($sr['role']) !== $req_role) {
                echo "<script>alert('❌ 상위 사용자 역할이 올바르지 않습니다. (필요 역할: {$req_role})'); window.history.back();</script>";
                exit;
            }
            $sponsor_id = $sid;
        }
    ...
    if ($new_role === 'investor' && $referrer_id_raw !== '') {
        if (!ctype_digit($referrer_id_raw)) {
            echo "<script>alert('❌ 추천인 선택값이 올바르지 않습니다.'); window.history.back();</script>";
            exit;
        }
...
$st = $conn->prepare("SELECT id FROM users WHERE id = ? AND role = 'investor'");
        ...
        $st->close();
        if (!$rr) {
            echo "<script>alert('❌ 선택한 추천인을 찾을 수 없습니다.'); window.history.back();</script>";
            exit;
        }
        $referrer_id = $rid;
    }
    ...
    try {
        ...
        echo "<script>alert('✅ 계정 생성 완료!'); window.location.href=" . json_encode($redirect_url) . ";</script>";
        exit;
    } catch (Throwable $e) {
        $conn->rollback();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (safe_post('_action') === 'update_user')) {
    ...
    if ($target_id <= 0) {
        echo "<script>alert('❌ 수정 대상 정보가 없습니다.'); window.location.href=" . json_encode($redirect_url) . ";</script>";
        exit;
    }
...
// layout
$page_title = "CREATE ACCOUNT (2-STEP)";
